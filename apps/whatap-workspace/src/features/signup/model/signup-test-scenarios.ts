...
export const TEST_SCENARIOS: TestScenarioEntry[] = [
  {
    ...
    flow: ['/signup (일반 폼)', '/signup/lead-info (빈 폼)', '/signup/verify', '/signup/success (워크스페이스 생성)'],
    scenario: {
      type: 'default',
      email: '',
    },
  },
  {
    ...
    badge: 'Invited',
    badgeColor: 'bg-blue-50 text-[#296cf2]',
    flow: [
      '/signup (email readonly)',
      '/signup/lead-info (프리필)',
      '/signup/verify',
      '/signup/success (워크스페이스 이동)',
    ],
    scenario: {
      type: 'invited-single',
  ...
  },
  {
    ...
    flow: [
      ...
      '/signup/lead-info (프리필)',
      '/signup/verify',
      '/signup/workspace-select (선택)',
      '/signup/success (대시보드 이동)',
    ],
    scenario: {
      type: 'invited-multi',
      email: 'alex.kim@whatap.io',
