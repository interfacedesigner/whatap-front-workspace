...
base branch 인자가 없으면 사용자에게 입력 요청:

```bash
git branch --show-current
```

브랜치명에서 Jira 이슈 태그 추출:

- `feature/*` 형식일 경우 `*` 부분이 JIRA_ISSUE_TAG가 됨
- 예: `feature/FRONT-323` → JIRA_ISSUE_TAG: `FRONT-323`
- 예: `feature/ARCHCOMMON-323` → JIRA_ISSUE_TAG: `ARCHCOMMON-323`
- `feature/` 접두사가 없으면 Jira 링크는 "없음"

PR 제목 생성:

- `feature/*` 형식일 경우 `[{JIRA_ISSUE_TAG}]` 형식으로 생성
- 예: `feature/FRONT-323` → `[FRONT-323]`
- 예: `feature/ARCHCOMMON-323` → `[ARCHCOMMON-323]`

### 2. Jira 이슈 정보 수집

브랜치가 `feature/`로 시작하는 경우:

- WebFetch로 `https://whatap-labs.atlassian.net/browse/{JIRA_TAG}` 접근
- Task tool (general-purpose subagent)를 사용하여 이슈 설명 요약
- 요약 내용을 "배경 (문제 & 원인)" 섹션에 작성
